...
export function useJanus (lockArg?: string, urlArg?: string) {
	...
	const request = curry(
		async (method: 'POST'|'PATCH', payload: Record<string, any>) => {
			...
			if (!load.success && method === 'POST') reset()

			return load
		}
	)
	const authentication = (body: Record<string, any>) => {
		if (!lock.value) return reset()
		return request('POST', { lock, ...body })
	}
	const action = (action: string) => (update?: UserUpdate) => request('PATCH', { token, action, update })
	const actions = {
	...
	const requestToken = (email: string) => authentication({ email, url })
	const login = async () => {
	...
	if (janus && !token.value) token.value = janus as string
